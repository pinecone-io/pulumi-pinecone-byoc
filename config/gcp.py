"""
Configuration for Pinecone BYOC GCP infrastructure.
"""
...
class AlloyDBInstanceConfig(BaseModel):
    """Configuration for a single AlloyDB instance."""
...
class AlloyDBConfig(BaseModel):
    """AlloyDB database configuration with control-db and system-db instances."""
...
class GCPConfig(BaseConfig):
    ...
    Extends BaseConfig with GCP-specific settings.
    """
    ...
    # Custom labels/tags (GCP uses labels, AWS uses tags - accept both names)
    custom_tags: dict[str, str] = Field(default_factory=dict)

    def labels(self, **extra: str) -> dict[str, str]:
        """Generate consistent resource labels, including user-provided custom labels."""
        base_labels = {
            "pinecone-managed-by": "pulumi",
        }
        return {**base_labels, **self.custom_tags, **extra}

    # Alias for cross-cloud compatibility (AWS uses tags, GCP uses labels)
    tags = labels
...
# Aliases used by gcp/cluster.py _build_config
GCPNodePoolConfig = NodePoolConfig
GCPDatabaseConfig = AlloyDBConfig
GCPDatabaseInstanceConfig = AlloyDBInstanceConfig
