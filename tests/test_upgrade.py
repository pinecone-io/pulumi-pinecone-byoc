...
SOURCE_PINECONE_VERSION = os.environ.get("UPGRADE_SOURCE_VERSION", "main-bb42e5b")
...
def pin_published_module(project_dir, ref):
    ...
    # generate_project already synced against the unpinned dependency, so without
    # this the baseline runs whatever latest resolved to and the pin means nothing
...
def test_upgrade_vanilla(request, baseline):
    ...
    try:
        ...
        # the baseline is already up by the time this test runs, so an upgrade that
        # fails before the copy is complete still has a deployment to tear down -
        # and either directory names the same stack
        project_dir = (
            candidate_dir if (candidate_dir / f"Pulumi.{stack}.yaml").exists() else baseline_dir
        )
        if keep_stacks(request):
            message = (
                f"leaving upgrade stack {stack} up - destroy it with: "
                f"pulumi destroy --yes -C {project_dir} --stack {stack}"
            )
            print(f"\n{message}")
            logging.info(message)
        else:
            delete_project_indexes(project_dir, stack)
