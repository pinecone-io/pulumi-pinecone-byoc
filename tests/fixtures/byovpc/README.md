...
picks that mode, so `ilia-carve`, `carve-ci` and `carve` all work. Set
`byovpc:mode` to override.

| mode    | creates             | exercises                                   |
| ------- | ------------------- | ------------------------------------------- |
| `carve` | no workload subnets | bring-your-own-VPC: the module creates the subnets |

NAT egress sits on the VPC **main route table**, because the subnets the module
creates carry no route table of their own and inherit it.
