...
def egress_check(vpc_id, azs, cidr):
    """The preflight against real AWS. The e2e harness runs the wizard headless and
    _run_headless never calls _run_preflight_checks, so nothing else here reaches it.
    """
    checker = AWSPreflightChecker(os.environ["AWS_REGION"], azs, cidr, vpc_id=vpc_id)
    checker._check_carve_egress()
    return checker.results[-1]


@pytest.mark.parametrize("byovpc", ["carve"], indirect=True)
def test_the_egress_preflight_passes_against_a_vpc_that_really_routes(byovpc):
    env = parse_wizard_env(byovpc["wizard_env"])

    result = egress_check(
        byovpc["vpc_id"],
        env["PINECONE_AZS"].split(","),
        env["PINECONE_VPC_CIDR"],
    )

    assert result.passed, result.message
    assert "nat-" in result.message, "the fixture egresses via a NAT and the check should say so"


def test_the_egress_preflight_fails_against_a_vpc_with_nowhere_to_route(customer_vpc):
    result = egress_check(
        customer_vpc["vpc_id"],
        customer_vpc["azs"].split(","),
        customer_vpc["carve_cidr"],
    )

    assert not result.passed, "a VPC with no default route would strand the EKS nodes"
